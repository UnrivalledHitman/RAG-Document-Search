...
class RAGNodes:
    ...
    def __init__(self, retriever, llm):
        ...
        self.llm = llm
        self.agent: Runnable = self._build_agent()  # build once, no lazy init
    ...
    def _build_tools(self) -> List[Tool]:
        ...
        wiki = WikipediaQueryRun(
            api_wrapper=WikipediaAPIWrapper(top_k_results=3, wiki_client=any)
        )
    ...
    def generate_answer(self, state: RAGState) -> RAGState:
        ...
        result = self.agent.invoke(
            input={"messages": [HumanMessage(content=state.question)]}
        )
